...
def filter_by_polygons(list_det, list_landmarks, polygons):
    ...
    for ib in range(len(list_det)): # Foreach image
        if len(list_det[ib]) > 0:
            ...
            list_new_det.append([])
            list_new_landmarks.append([])
    return list_new_det, list_new_landmarks

class RetinafaceBatchTRT(object):
    def __init__(self,
                model_path='weights/retinaface_r50_v1-1x3x640x640.trt',
                input_shape = (640, 640),
                batch_size = 1,
    ...
    def warm_up(self):
        ...
        tik = time.time()
        image = cv2.imread('lumia.jpg', cv2.IMREAD_COLOR)
...
class SCRFDBatchTRT:
    def __init__(self,
                model_path='weights/scrfd-finetuning-640x640-batchsize-1-fp32.trt',
                input_shape = (640, 640),
                batch_size = 1, # Current support only batchsize = 1
                engine = 'TRT', 
                triton_input_names = None,
        ...
        if self.engine == 'TRT':
            ...
            self.model = tritonclient.grpc.InferenceServerClient(model_path)
    ...
    def forward(self, batch_img, thresh):
        ...
        if self.engine == 'TRT':
            ...
            inputs = [
                tritonclient.grpc.InferInput(self.triton_input_names[0], batch_img.shape, np_to_triton_dtype(np.float32))
            ]
            inputs[0].set_data_from_numpy(np.float32(batch_img))
            outputs = [
...
if __name__ == '__main__':
    ...
    model = RetinafaceBatchTRT(model_path='weights/retinaface_r50_v1.onnx',
                            ...
                            post_process_type = 'SINGLE',
                            engine = 'ONNX')
    # model = SCRFDBatchTRT(model_path='weights/scrfd.onnx',
    #             input_shape = (640, 640),
    #             batch_size = 1, # Current support only batchsize = 1
    #             engine = 'ONNX')
